...
void HTMLSelectElement::update_cached_list_of_options() const
{
    ...
    auto is_nested_optgroup = [&](DOM::Node const& node) {
...
void HTMLSelectElement::set_selected_index(WebIDL::Long index)
{
    ...
    for (auto& option : m_cached_list_of_options) {
        ...
        // 2. If firstMatchingOption is null and option's index is equal to the given value, then set firstMatchingOption to option.
        if (!first_matching_option && option->index() == static_cast<int>(index))
            first_matching_option = option;
    }

    // 3. If firstMatchingOption is not null, then set firstMatchingOption's selectedness to true and set firstMatchingOption's dirtiness to true.
...
WebIDL::ExceptionOr<void> HTMLSelectElement::set_value(Utf16String const& value)
{
    ...
    for (auto const& option_element : m_cached_list_of_options) {
        ...
        // 2. If firstMatchingOption is null and option's value is equal to the given value, then set firstMatchingOption to option.
...
GC::Ptr<HTMLSelectedContentElement> HTMLSelectElement::enabled_selectedcontent() const
{
    ...
    // 2. Let selectedcontent be the first selectedcontent element descendant of select in tree order if any such element exists; otherwise return null.
...
WebIDL::ExceptionOr<void> HTMLSelectElement::update_selectedcontent()
{
    ...
    // 3. Let option be the first option in select's list of options whose selectedness is true, if any such option exists, otherwise null.
    update_cached_list_of_options();
