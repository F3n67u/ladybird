...
// Parses a MIME type string and determines whether it is a web custom format.
static std::pair<Optional<MimeSniff::MimeType>, bool> parse_mime_type_and_flag_custom(String const& type)
{
    // 2. Let isCustom be false.
    bool is_custom = false;
    auto mime_type_without_prefix = type;
    ...
    if (type.starts_with_bytes(WEB_CUSTOM_FORMAT_PREFIX)) {
        ...
        mime_type_without_prefix = MUST(type.substring_from_byte_offset(WEB_CUSTOM_FORMAT_PREFIX.length()));
    ...
    auto mime_type = MimeSniff::MimeType::parse(mime_type_without_prefix);

    return { mime_type, is_custom };
}

// https://w3c.github.io/clipboard-apis/#dom-clipboarditem-clipboarditem
WebIDL::ExceptionOr<GC::Ref<ClipboardItem>> ClipboardItem::construct_impl(JS::Realm& realm, OrderedHashMap<String, ClipboardItemData> const& items, ClipboardItemOptions const& options)
{
    ...
    for (auto const& [key, value] : items) {
        ...
        auto [mime_type, is_custom] = parse_mime_type_and_flag_custom(key);
        ...
        String mime_type_serialized = mime_type->serialized();
        ...
        String mime_type_string = is_custom ? MUST(String::formatted("{}{}", WEB_CUSTOM_FORMAT_PREFIX, mime_type_serialized)) : mime_type_serialized;

        // 13. Add mimeTypeString to types.
        types.append(mime_type_string);
        ...
        Representation representation { move(mime_type_serialized), is_custom, value };
        clipboard_item->m_representations.append(representation);
    }
...
WebIDL::ExceptionOr<GC::Ref<WebIDL::Promise>> ClipboardItem::get_type(String const& type)
{
    ...
    //      2. Set isCustom to true.
    // 4. Let mimeType be the result of parsing a MIME type given type.
    auto [mime_type, is_custom] = parse_mime_type_and_flag_custom(type);
